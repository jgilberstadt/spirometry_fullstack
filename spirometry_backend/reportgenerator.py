...
def generate(filename, patient_id, host='localhost'):
	...
	for i in xrange(len(max_fev_list)):
		...
		worksheet.write(
			row, col+9, round(((max_fev_list[i]-overall_max)/float(overall_max))*100, 2))
		worksheet.write(row, col+10, round(overall_mean, 2))
		worksheet.write(row, col+11, round(((max_fev_list[i]-overall_mean)/float(overall_mean))*100, 2))
